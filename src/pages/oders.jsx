...
import { addDoc, collection, getDocs, getDoc, Timestamp, doc, query, where } from "firebase/firestore";
...
const OrderSummary = () => {
  ...
  const handleConfirmOrder = useCallback(async () => {
    ...
    try {
      ...
      const pointsUsed = usePoints ? Math.ceil((total + deliveryFee) / CREDIT_PER_POINT) : 0;
      ...
      await Promise.all([
        addDoc(collection(db, "notifications"), {
          ...
          read: false,
          message: `Nouvelle commande #${orderRef.id.slice(0, 6)}${usePoints ? " (paiement avec points)" : ""}`,
        ...
        usePoints &&
          addDoc(collection(db, "pointsTransactions"), {
            ...
            timestamp: Timestamp.now(),
            read: false,
            message: `Demande d'utilisation de ${formatPrice(pointsUsed)} pts pour commande #${orderRef.id.slice(0, 6)}`,
        ...
        loyaltyPoints > 0 &&
          ...
          addDoc(collection(db, "pointsTransactions"), {
            ...
            timestamp: Timestamp.now(),
            read: false,
            message: `Demande d'octroi de ${formatPrice(loyaltyPoints)} pts pour commande #${orderRef.id.slice(0, 6)} après confirmation du paiement`,
            type: "points_grant",
          }),
      ].filter(Boolean));
      ...
      if (err.code === "unavailable") {
        ...
        setError(err.message || "Erreur lors de la soumission de la commande. Veuillez vérifier votre connexion ou réessayer.");
      }
    } finally {
      setLoading(false);
    }
  }, [cartItems, normalizedAddress, normalizedPayment, total, deliveryFee, usePoints, isGuest, contact, clearCart, navigate, loyaltyPoints, isOnline]);
  ...
  const canUsePoints = userPoints >= Math.ceil((total + getDeliveryFee(normalizedAddress?.area)) / CREDIT_PER_POINT) && !isGuest;
    ...
    <div className="min-h-screen bg-gray-100 pb-20">
          ...
          {usePoints && (
            <div className="mt-2 text-sm text-gray-600">
              <p>
                ...
                {formatPrice(Math.ceil((total + getDeliveryFee(normalizedAddress?.area)) / CREDIT_PER_POINT))}
              </p>
              <p className="text-xs">
                Votre demande sera soumise à approbation. Les points seront déduits après confirmation par l'admin.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-3 shadow-lg">
        <button
          ...
          disabled={loading || (!isOnline && normalizedPayment?.id === "payment_mobile")}
          aria-label={loading ? "Validation en cours" : usePoints ? "Confirmer avec points" : "Confirmer la commande"}
        ...
        >
          ...
          ) : usePoints ? "Confirmer avec mes points" : "Confirmer la commande"}
        </button>
      </div>
    </div>
  );
